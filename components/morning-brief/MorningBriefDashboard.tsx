...
export function MorningBriefDashboard({ persisted, development = false }: { persisted?: Partial<Record<string, MorningBriefStory[]>> | null; development?: boolean }) { const [activeCategory, setActiveCategory] = useState<BriefCategory>("top5"); const active = categoryTabs.find((tab) => tab.id === activeCategory)!; const key = activeCategory === "top5" ? "top_5" : activeCategory === "em-frontier" ? "emerging_frontier" : activeCategory === "vc-pe" ? "vc_pe" : activeCategory; const stories = persisted?.[key] ?? feedByCategory[activeCategory]; return <div className="mx-auto max-w-4xl space-y-7 pb-8"><div className="flex justify-end">{development && <MockBriefGenerator />}</div><MorningBriefHeader /><MarketPulse /><MorningSummary /><BriefCategoryTabs active={activeCategory} onChange={setActiveCategory} /><StoryFeed category={activeCategory} title={active.title} subtitle={active.subtitle} stories={stories} /></div>; }
