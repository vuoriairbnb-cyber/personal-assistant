...
export function MorningBriefDashboard({ persisted, marketQuotes = [], dailyIntelligence = null }: { persisted?: Partial<Record<string, MorningBriefStory[]>> | null; marketQuotes?: MarketQuote[]; dailyIntelligence?: DailyIntelligence | null }) { const [activeCategory, setActiveCategory] = useState<BriefCategory>("top5"); const active = categoryTabs.find((tab) => tab.id === activeCategory)!; const key = activeCategory === "top5" ? "top_5" : activeCategory === "em-frontier" ? "emerging_frontier" : activeCategory === "vc-pe" ? "vc_pe" : activeCategory; const stories = persisted?.[key] ?? feedByCategory[activeCategory]; return <div className="mx-auto max-w-4xl space-y-7 pb-8"><div className="flex justify-end"><MockBriefGenerator /></div><MorningBriefHeader /><MarketPulse quotes={marketQuotes} /><DailyIntelligenceBrief intelligence={dailyIntelligence} /><MorningSummary /><BriefCategoryTabs active={activeCategory} onChange={setActiveCategory} /><StoryFeed category={activeCategory} title={active.title} subtitle={active.subtitle} stories={stories} /></div>; }
