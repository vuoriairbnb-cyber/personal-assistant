...
export async function getKideEvent(eventId: string): Promise<KideEvent> {
  const response = await fetch(`${API_BASE}/${encodeURIComponent(eventId)}`, { headers: { Authorization: `Bearer ${token()}` }, cache: "no-store" });
  if (!response.ok) {
  ...
  let payload: unknown;
  try { payload = await response.json(); } catch { throw new KideError("Kide palautti virheellisen vastauksen.", 502); }
  try { return normalizeKideEvent(payload); } catch { throw new KideError("Kide palautti virheellisen tapahtuman.", 502); }
}
