...
const playerWatches = { processed: 0, newMatches: 0, failed: 0 };
...
test("authorized POST calls the watch processor and returns its summary", async () => {
  ...
  assert.deepEqual(await response.json(), { ok: true, ...watches, player_watches: playerWatches, notifications, duration_ms: 10 });
});
