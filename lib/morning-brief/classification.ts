...
export async function classifyImportedArticle(article: ExtractedArticle, options: { timeoutMs?: number } = {}) {
  ...
  try { luna = await classifyWithModel(article, models.default, options.timeoutMs); }
    ...
    return { ...terra, modelsUsed: [models.default, models.advanced], escalationReason: "invalid_structure" as const };
  }
  ...
  if (!escalationReason) return { ...luna, modelsUsed: [luna.model], escalationReason: null };
  const terra = await classifyWithModel(article, models.advanced, options.timeoutMs);
  return { ...terra, modelsUsed: [luna.model, terra.model], escalationReason: escalationReason as ClassificationEscalationReason };
}
