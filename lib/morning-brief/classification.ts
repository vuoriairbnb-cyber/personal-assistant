...
export async function classifyImportedArticle(article: ExtractedArticle) {
  const lunaModel = classificationModelRoute(100)[0]!; const luna = await classifyWithModel(article, lunaModel);
  // Ambiguous or limited-content classification gets one permitted Terra retry; never Sol.
  const terraModel = classificationModelRoute(luna.data.confidence)[1]; return terraModel ? classifyWithModel(article, terraModel) : luna;
}
