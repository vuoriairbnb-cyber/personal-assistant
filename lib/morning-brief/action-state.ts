...
export function formatIngestionSummary(sources: SourceIngestionSummary[]) {
  const details = sources.map((source) => source.error ? `${source.source}: ${source.error}` : `${source.source}: ${source.fetched} fetched, ${source.new} new, ${source.duplicates} duplicates, ${source.filtered} filtered, ${source.skipped} skipped`).join(" · ");
