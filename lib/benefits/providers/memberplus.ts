...
export function normalizeMemberPlusBenefit(raw: RawBenefit, unions: Map<string, string>): NormalizedBenefit | null {
  ...
  return { provider: "memberplus", externalId, title, heading, description, providerCategory: category, normalizedCategory: null, offerer: string(raw.itemOfferer), locations: [], restrictions: null, benefitText, redemptionType: redemption.type, redemptionCode: redemption.code, redemptionUrl: redemption.url, sourceUrl: string(raw.url), imageUrl: string(raw.benefitImage), validFrom: normalizedDate(raw.itemStartDate), validUntil: normalizedDate(raw.itemEndDate), unionId, unionName: unionId ? unions.get(unionId) ?? null : null, isGeneralBenefit: !unionId, isRecommended: raw.recommended === true, providerMetadata: null, searchText };
}
